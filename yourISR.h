...
extern alt_u32 switch6_id;
extern alt_u32 switch7_id;
extern alt_u32 switch8_id;
...
//see main.c for explanations of the following variables
extern int leftDelay;
extern int rightDelay;
extern int leftIntensity;
extern int rightIntensity;
extern int sound_intensity;
extern int sound_lowPassIndex;
...
static void handle_key0_interrupt(void* context, alt_u32 id) {
	...
	*key0ptr = IORD_ALTERA_AVALON_PIO_EDGE_CAP(KEY0_BASE);
	...
	for (i = 0; i < RX_BUFFER_SIZE; i++) {
		int temp = (RxHead + i) % RX_BUFFER_SIZE;
		printf("[%d] = %d, ", i, rx_buffer[temp]);
	}
	printf("\n");
}
...
static void handle_leftready_interrupt_test(void* context, alt_u32 id) {
	...
	*leftreadyptr = IORD_ALTERA_AVALON_PIO_EDGE_CAP(LEFTREADY_BASE);
	...
	//newReadIndex implements the delay factor
	int newReadIndex = (leftCount + leftDelay) % BUFFERSIZE;
	//average the channels in order to combine the jungle and fanfare sounds
	int out = leftChannel + leftIntensity*(rightChannelData[newReadIndex])/100;
	IOWR_ALTERA_AVALON_PIO_DATA(LEFTSENDDATA_BASE, out);
...
static void handle_rightready_interrupt_test(void* context, alt_u32 id) {
	...
	*rightreadyptr = IORD_ALTERA_AVALON_PIO_EDGE_CAP(RIGHTREADY_BASE);
	...
	//newReadIndex implements the delay factor
	int newReadIndex = (rightCount + rightDelay) % BUFFERSIZE;
	//average the channels in order to combine the jungle and fanfare sounds
	int out =  leftChannel + rightIntensity*(rightChannelData[newReadIndex])/100;

	IOWR_ALTERA_AVALON_PIO_DATA(RIGHTSENDDATA_BASE, out);
